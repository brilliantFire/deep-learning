...
# (IN DEVELOPMENT) This program builds a deep neural network model of eye state
...
X_trainArr = numpy.array(X_train)
y_trainArr = numpy.array(y_train)
X_testArr = numpy.array(X_test)
...
# Fit the model
    ...
    # validation
model.fit(X_trainArr, y_trainArr, epochs=30000, batch_size=100, 
...
# on?
# First, calculate predictions and round them (50% decision rule)
preds = model.predict(y_testArr)
roundPreds = [round(x[0]) for x in preds]
